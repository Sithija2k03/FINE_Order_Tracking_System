...
interface Picker { id: string; name: string; department: string; }
...
export default function PickerSelectPage() {
  ...
  const fetchPickers = async (dept: string) => {
    setLoading(true);
    try {
      const res = await api.get(`/pickers?department=${dept}`);
      setPickers(res.data);
    ...
    return (
      <div className="min-h-screen bg-blue-950 flex flex-col items-center justify-center p-4 md:p-6">
        <div className="text-center mb-8 md:mb-12">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-white">Picker Login</h1>
  ...
  return (
    <div className="min-h-screen bg-blue-950 p-4 md:p-6">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Back button */}
        <button
          onClick={() => { setDepartment(null); setPickers([]); }}
        ...
        {!loading && pickers.length === 0 && (
          <div className="text-center text-blue-300 py-12 text-lg">
            No pickers found for this department
          </div>
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {pickers.map(p => (
            <button
              key={p.id}
              onClick={() => navigate(`/picker/${p.id}?dept=${department}`)}
            ...
            >
              ...
              <span className="text-sm md:text-base text-center leading-tight">{p.name}</span>
            </button>
          ))}
        </div>
